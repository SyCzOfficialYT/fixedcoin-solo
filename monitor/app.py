    ...
    if len(out)<5:
        for raw in read_tail_lines(STRATUM_LOG,300):
            if any(x in raw for x in ("ACCEPT","BLOCK","ERROR","REJECT","authorize","NEW ROUND")):
                out.append({"ts":raw[:19],"level":"OK" if "ACCEPT" in raw or "BLOCK" in raw else ("WARN" if "REJECT" in raw else "INFO"),"msg":raw[20:].strip()})
    return out[-limit:]
...
def fmt_duration(sec):
    ...
    return f"{d}d {h}h" if d else (f"{h}h {m}m" if h else f"{m}m {s:02d}s")
...
def resolve_pool_diff(s):
    ...
    for x in reversed(s.get("recent_shares") or []):
        try:
            v=float(x.get("pool_diff") or x.get("diff") or 0)
            if v>0:return v
        except Exception:pass
...
def estimate_hashrate(s,pd):
    ...
    if not a:return 0.0
    total=sum(float(x.get("pool_diff") or x.get("diff") or pd or 0) for x in a)
    ts=[_parse_ts(x.get("ts") or x.get("time")) for x in a]; ts=[x for x in ts if x]
    if not total:return 0.0
    span=max(1.0,(max(ts)-min(ts)).total_seconds()) if len(ts)>1 else max(5.0,5*len(a))
    return total*(2**32)/span
