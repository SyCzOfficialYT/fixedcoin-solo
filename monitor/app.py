...
def parse_stratum_log():
    ...
    auth_re=re.compile(r"authorize\s+(\S+)\s+diff=(\d+(?:\.\d+)?)"); accept_re=re.compile(r"(\d{4}-\d\d-\d\d \d\d:\d\d:\d\d).*?ACCEPT\s+#(\d+)\s+work=([0-9.]+).*?pool=([0-9.]+).*?round=([0-9.]+)%\s+hash=([0-9a-fA-F]+)"); reject_re=re.compile(r"REJECT\s+low difficulty\s+need=([0-9.]+)\s+work=([0-9.]+)\s+hash=([0-9a-fA-F]+)"); round_re=re.compile(r"NEW ROUND\s+height=(\d+)\s+netdiff=([0-9.]+)"); block_re=re.compile(r"(?:BLOCK FOUND|BLOCKFOUND|FOUND BLOCK).*?(?:height[= ](\d+))?.*?(?:hash[= ]([0-9a-fA-F]{16,64}))?.*?(?:reward[= ]([0-9.]+))?",re.I)
    for line in lines:
...
def estimate_hashrate(s,pd):
    ...
    if not a:return 0.0
    total=sum(float(x.get("pool_diff") or x.get("diff") or pd or 0) for x in a); ts=[_parse_ts(x.get("ts") or x.get("time")) for x in a]; ts=[x for x in ts if x]; span=max(1.0,(max(ts)-min(ts)).total_seconds()) if len(ts)>1 else max(5.0,5*len(a)); return total*(2**32)/span if total else 0.0
def wallet_balances():
...
def api_health():r,_=rpc("getblockchaininfo");return jsonify({"ok":True,"rpc_ok":bool(r),"ts":time.strftime("%Y-%m-%d %H:%M:%S")})
@app.route("/api/logs")
def api_logs():
