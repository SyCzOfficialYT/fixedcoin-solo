...
def rpc(method, params=None, timeout=None):
    try:
        r = requests.post(f"http://{RPC_HOST}:{RPC_PORT}", json={"jsonrpc":"1.0","id":"dash","method":method,"params":params or []}, auth=HTTPBasicAuth(RPC_USER, RPC_PASS), timeout=DASH_RPC_TIMEOUT if timeout is None else timeout)
...
def parse_stratum_log():
    lines = read_tail_lines(STRATUM_LOG); accepted=[]; rejected=0; workers={}; current_worker=None; current_diff=0.0; round_height=0; round_netdiff=0.0; round_work=0.0; round_shares=0; best_work=0.0; last=None; blocks=[]
    auth_re=re.compile(r"authorize\s+(\S+)\s+diff=([0-9.]+)", re.I)
    accept_re=re.compile(r"(\d{4}-\d\d-\d\d \d\d:\d\d:\d\d).*?ACCEPT\s+#(\d+)\s+work=([0-9.]+).*?pool=([0-9.]+).*?round=([0-9.]+)%\s+hash=([0-9a-fA-F]+)", re.I)
    reject_re=re.compile(r"REJECT\s+low difficulty\s+need=([0-9.]+)\s+work=([0-9.]+)\s+hash=([0-9a-fA-F]+)", re.I)
    round_re=re.compile(r"NEW ROUND\s+height=(\d+)\s+netdiff=([0-9.]+)", re.I)
    ...
    for line in lines:
        ...
        if m:
            current_worker=m.group(1); current_diff=float(m.group(2)); workers.setdefault(current_worker,{"ok":0,"bad":0,"diff":current_diff})["diff"]=current_diff; continue
        m=round_re.search(line)
        if m:
            round_height=int(m.group(1)); round_netdiff=float(m.group(2)); round_work=round_shares=0; best_work=0.0; continue
        m=accept_re.search(line)
        if m:
            ts,num,work,pool_diff,rpct,h=m.groups(); work=float(work); pool_diff=float(pool_diff)
            last={"ts":ts,"num":int(num),"work":work,"pool_diff":pool_diff,"net_pct":100*work/round_netdiff if round_netdiff else 0,"round_pct":float(rpct),"hash":h,"height":round_height or None}
            accepted.append(last); round_work+=work; round_shares+=1; best_work=max(best_work,work)
...
def maturity_info(height,items):
    ...
    for b in items or []:
        ...
        if not bh: continue
        mature_at=int(b.get("mature_at_height") or bh+COINBASE_MATURITY); left=max(0,mature_at-height)
        out.append({**b,"confs":min(COINBASE_MATURITY,max(0,height-bh)),"remaining":left,"left":left,"mature":left==0,"spendable":left==0,"mature_at_height":mature_at})
    return out
...
def wallet_balances():
    ...
    for method in ("getbalances","getwalletinfo"):
        ...
        if method=="getbalances":
            ...
            return {"confirmed":float(mine.get("trusted") or 0),"unconfirmed":float(mine.get("untrusted_pending") or 0)+float(mine.get("immature") or 0),"wallet_loaded":loaded,"wallet_error":werr}
