#!/usr/bin/env python3
"""FixedCoin Solo dashboard: FreeCash NEW-FCH UI backed by FixedCoin RPC/wallet."""
...
def parse_stratum():
    ...
    for line in read_lines(STRATUM_LOG):
        # Authorization formats used by the FreeCash-derived stratum.
        m=re.search(r"authorize\s+(\S+).*?(?:diff|share_diff)\s*[=:]\s*([0-9.]+)",line,re.I)
        if m:
            current_worker=m.group(1); current_diff=float(m.group(2)); workers.setdefault(current_worker,{"ok":0,"bad":0,"diff":current_diff})
        ...
        # Accept both the explicit block message and candidate/submission wording.
...
def payload():
    info,info_err=rpc("getblockchaininfo"); net,_=rpc("getnetworkinfo"); s=load_stats(); log=parse_stratum()
    # Log-derived mining state is authoritative for live shares; persisted stats fills gaps.
    shares=log if log["shares_ok"] or log["shares_bad"] else s
    ...
    confirmed,pending,immature,werr=wallet_balances(); wblocks,_=wallet_blocks(); found_blocks=list(log.get("blocks_log") or [])
    # Merge wallet/chain/log records by txid/hash/height. A freshly accepted block is visible immediately,
    # then wallet/chain data replaces it as soon as Core indexes the coinbase.
    merged={str(x.get("txid") or x.get("hash") or x.get("height")):x for x in wblocks}
    ...
    blocks=sorted(merged.values(),key=lambda x:int(x.get("height") or 0),reverse=True)[:1000]
    rewards=sum(float(x.get("reward") or 0) for x in blocks)
    effort=min(100,100*best/nd) if nd else 0; last_pct=min(100,100*last/nd) if nd else 0; eta=(nd*2**32/hr) if hr else None
    return {"synced":bool(info) and not bool((info or {}).get("initialblockdownload",False)),"height":height,"headers":headers,"difficulty":diff,"difficulty_fmt":fmt_diff(diff),"hashrate_fmt":fmt_hr(hr),"balance_fmt":f"{confirmed:.8f}","immature_fmt":f"{immature:.8f}","confirmed":confirmed,"confirmed_fmt":f"{confirmed:.8f}","unconfirmed":pending+immature,"unconfirmed_fmt":f"{pending+immature:.8f}","blocks_found":len(blocks),"rewards_fmt":f"{rewards:.8f}","effort_pct":round(effort,4),"best_pct":round(effort,4),"last_pct":round(last_pct,4),"eta_fmt":fmt_time(eta),"best_share_fmt":fmt_diff(best),"last_share_work_fmt":fmt_diff(last),"shares_ok":int(shares.get("shares_ok") or 0),"shares_bad":int(shares.get("shares_bad") or 0),"reject_pct":round(100*int(shares.get("shares_bad") or 0)/max(1,int(shares.get("shares_ok") or 0)+int(shares.get("shares_bad") or 0)),1),"share_diff_fmt":fmt_diff(float(POOL.get("fixed_difficulty") or POOL.get("start_difficulty") or 1)),"last_share_time":shares.get("last_share_time"),"last_share_hash":shares.get("last_share_hash"),"payout":PAYOUT,"addr_ok":bool(PAYOUT),"workers":shares.get("workers") or {},"connections":int((net or {}).get("connections") or 0),"rpc_ok":bool(info),"rpc_error":None if info else info_err,"recent_shares":list(reversed(recent)),"blocks_log":blocks,"maturity_blocks":COINBASE_MATURITY,"ts":time.strftime("%Y-%m-%d %H:%M:%S")}
...
def events(limit=160):
    ...
    for raw in read_lines(STRATUM_LOG,300):
        ...
        level="OK" if "ACCEPT" in raw or "BLOCK" in raw else "WARN" if "REJECT" in raw else "ERROR" if "ERROR" in raw else "INFO"
        out.append({"ts":raw[:19],"level":level,"msg":raw[20:].strip() if len(raw)>20 else raw})
    return out[-limit:]
...
@app.route("/")
def index():
    p=payload(); return make_response(render_template("dashboard.html",**p).replace("FreeCash","FixedCoin").replace("FCH","FIX"))
