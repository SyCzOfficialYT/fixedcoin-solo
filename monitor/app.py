#!/usr/bin/env python3
"""FixedCoin Solo dashboard — exact FreeCash UI contract, FixedCoin data."""
...
ROOT=Path(__file__).resolve().parent.parent; CONFIG_PATH=ROOT/"config"/"config.yaml"; EVENTS_PATH=Path(os.environ.get("EVENTS_PATH",str(ROOT/"data"/"events.jsonl"))); STRATUM_LOG=Path(os.environ.get("STRATUM_LOG",str(ROOT/"data"/"stratum.log"))); STATS_PATH=Path(os.environ.get("STATS_PATH",str(ROOT/"data"/"stats.json"))); COINBASE_MATURITY=int(os.environ.get("COINBASE_MATURITY","100")); TARGET_BLOCK_SEC=int(os.environ.get("TARGET_BLOCK_SEC","60")); DASH_RPC_TIMEOUT=float(os.environ.get("DASH_RPC_TIMEOUT","5")); _CHAIN_CACHE={"ts":0.0,"height":-1,"rows":[]}
app=Flask(__name__,template_folder="templates")
def load_cfg():
    try:return yaml.safe_load(CONFIG_PATH.read_text()) or {}
    except Exception:return {}
CFG=load_cfg(); RPC=CFG.get("rpc") or {}; POOL=CFG.get("pool") or {}; RPC_HOST=RPC.get("host","127.0.0.1"); RPC_PORT=int(RPC.get("port",24761)); RPC_USER=RPC.get("user","fixrpc"); RPC_PASS=RPC.get("password",""); HOLDING=str(POOL.get("payout_address") or "").strip()
def rpc(method,params=None,timeout=None):
    try:
        r=requests.post(f"http://{RPC_HOST}:{RPC_PORT}",json={"jsonrpc":"1.0","id":"dashboard","method":method,"params":params or []},auth=HTTPBasicAuth(RPC_USER,RPC_PASS),timeout=DASH_RPC_TIMEOUT if timeout is None else timeout); r.raise_for_status(); d=r.json()
...
def _wallet_rows(limit=1000):
    ...
    for tx in txs:
        ...
        txid=str(tx.get("txid") or ""); bh=str(tx.get("blockhash") or ""); key=txid or f"{h}:{bh}"; row=rows.setdefault(key,{"ts":time.strftime("%Y-%m-%d %H:%M:%S",time.gmtime(tx.get("blocktime") or tx.get("time") or time.time())),"height":h,"hash":bh[:16] if bh else "–","txid":txid,"reward":0.0,"address":HOLDING or addr,"mature_at_height":h+COINBASE_MATURITY,"confirmations":0,"category":cat}); row["reward"]+=amount; row["confirmations"]=max(row["confirmations"],int(tx.get("confirmations") or 0)); row["category"]="generate" if cat=="generate" else row["category"]
    return list(rows.values()),None
def _chain_rows(seed_rows,height):
    now=time.time()
    if now-_CHAIN_CACHE["ts"]<10 and _CHAIN_CACHE["height"]==height:return list(_CHAIN_CACHE["rows"])
    heights=set()
    for b in seed_rows:
    ...
    for h in sorted(heights):
        ...
        key=tx.get("txid") or bh; conf=max(0,height-h+1); rows[key]={"ts":time.strftime("%Y-%m-%d %H:%M:%S",time.gmtime(blk.get("time") or time.time())),"height":h,"hash":bh[:16],"txid":tx.get("txid",""),"reward":reward,"address":HOLDING,"mature_at_height":h+COINBASE_MATURITY,"confirmations":conf,"category":"generate" if conf>COINBASE_MATURITY else "immature"}
    result=list(rows.values()); _CHAIN_CACHE.update({"ts":now,"height":height,"rows":result}); return result
def wallet_coinbase_log(height):
    rows,err=_wallet_rows(); chain=_chain_rows(rows,height) if err is None else []; merged={str(r.get("txid") or r.get("hash") or r.get("height")):r for r in rows}
...
def maturity_info(height,rows):
    ...
    for b in rows or []:
        ...
        if not h:continue
        mature_at=int(b.get("mature_at_height") or h+COINBASE_MATURITY); left=max(0,mature_at-height); out.append({**b,"confs":min(COINBASE_MATURITY,max(0,height-h+1)),"left":left,"remaining":left,"spendable":left==0,"mature":left==0,"mature_at_height":mature_at})
    return out
def fmt_diff(v):
    try:v=float(v)
...
def build_payload():
    s=merge_stats(load_stats(),parse_stratum_log()); info,info_err=rpc("getblockchaininfo"); net,_=rpc("getnetworkinfo"); height=int((info or {}).get("blocks") or s.get("round_height") or 0); headers=int((info or {}).get("headers") or height); difficulty=float((info or {}).get("difficulty") or s.get("network_diff") or 0); nd=difficulty or float(s.get("network_diff") or 1); ok=int(s.get("shares_ok") or 0); bad=int(s.get("shares_bad") or 0); total=ok+bad; pd=float(POOL.get("fixed_difficulty") or POOL.get("start_difficulty") or 1)
    for x in reversed(s.get("recent_shares") or []):
        try:
            q=float(x.get("pool_diff") or 0)
            if q>0:pd=q;break
        except Exception:pass
    best=float(s.get("best_share_diff") or 0); last=float(s.get("last_share_work") or 0); ts=[parse_ts(x.get("ts")) for x in s.get("recent_shares") or []]; ts=[x for x in ts if x]; span=max(1,(max(ts)-min(ts)).total_seconds()) if len(ts)>1 else 5; hr=sum(float(x.get("pool_diff") or pd) for x in s.get("recent_shares") or [])*(2**32)/span if s.get("recent_shares") else 0; wb=wallet_balances(); rows,werr=wallet_coinbase_log(height); mat=maturity_info(height,rows); found=len(rows); effort=min(100,100*best/nd) if nd and best else 0; last_pct=min(100,100*last/nd) if nd and last else 0; eta=(nd*2**32/hr) if hr>0 else None; synced=bool(info) and not bool((info or {}).get("initialblockdownload",False)) and height>=max(headers-1,0)
...
def load_events(limit=160):
    ...
    if EVENTS_PATH.exists():
        for raw in read_tail_lines(EVENTS_PATH,limit):
            try:e=json.loads(raw); out.append(e if isinstance(e,dict) else {})
            except Exception:pass
